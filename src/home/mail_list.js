...
export const SocialLink = ({ link }) => {
  return (
    <a href={link.href} target="_blank" rel="noreferrer" className={`${link.color} px-3 py-2 text-xs font-semibold text-white inline-flex items-center space-x-2 rounded`}>
      {link.svg}
      <span>{link.name}</span>
    </a>
  );
...
export const MailForm = () => {
  ...
  return (
    ...
    >
      <div className="mt-8 text-left lg:flex lg:mt-0 lg:flex-shrink-0 lg:px-8">
        <div className="lg:inline-flex rounded-md">
          <input
            id="email"
            type="email"
            name="data[Email]"
        ...
        <div className="lg:ml-3 lg:inline-flex rounded-md lg:shadow-sm">
          <button
